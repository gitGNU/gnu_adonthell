...
void win_label::set_font(win_font * tmp)
{
  font=tmp;
  if(font)
    init_draw();
...
void win_label::set_text(char * tmp)
{
  if(font)
    {    
      strcpy(texte,tmp);
  ...
  else {cout << "You must set font\n";exit(1); }
}

bool win_label::add_text(char * tmp)
{
...
char * win_label::get_text()
{
  if(size_texte==0) return NULL;
  else return texte;    
}
void win_label::clear_text()
{
  set_text("");
}

void win_label::set_auto_size(bool tmp)
{
  auto_size=tmp;
  if(auto_size) auto_height=false;
  init_draw();
  if(wc) wc->find_obj_max_y(); //for scrollbar
}

void win_label::set_auto_height(bool tmp)
{
  auto_height=tmp;
  if(auto_height) auto_size=false;
  init_draw();
  if(wc) wc->find_obj_max_y(); //for scrollbar
}

void win_label::set_cursor(win_cursor * tmp)
{
  if(tmp)
    {
      if(cursor) delete cursor;
  ...
  if(auto_size)
      ...
      while(i<size_texte && texte[i]!='\0')
	{
	  if(texte[i]=='\n')
	    ...
	    {
	      if(texte[i]==' ')tmp_length+=font->length; 
	      else if(font->in_table(texte[i]))tmp_length+=font->table[texte[i]].length;
	    }
	  i++;
	}
      if(max_length<tmp_length) max_length=tmp_length;
      ...
      if(auto_height)
	{
	  ...
	  u_int16 tmp_pos,tmp_pos2,tmp_y=0;
	  i=0;
	  while(i<size_texte && texte[i]!='\0')
	    {
	      begin_word=i;
	      ...
	      else
		{
	      ...
	      i=tmp_pos;
	      if(texte[i]==' ') use_space+=font->length;
	      else if(texte[i]=='\n')
		{
	      ...
	      while(tmp_pos2<tmp_pos)
		{
		  if(font->in_table(texte[tmp_pos2]))
			...
			use_space+=font->table[texte[tmp_pos2++]].length;
		      ...
		      {
			use_space=0+x_pad_l;tmp_y+=font->height;
			tmp_draw->putbox_img(&(font->table[texte[tmp_pos2]]),use_space,tmp_y);
			use_space+=font->table[texte[tmp_pos2++]].length;
		      }
		  else tmp_pos2++;
...
void win_label::resize(u_int16 l, u_int16 h)
{
  win_base::resize(l,h);
  if(font) init_draw();
  else 
...
void win_label::attach_select(win_select *tmp)
{
  wselect=tmp;
}

void win_label::dettach_select()
{
  if(wselect)
    {
      wselect->remove(this);
...
void win_label::set_select_mode(u_int8 tmp)
{
  win_base::set_select_mode(tmp);
  if(select_mode==WIN_SELECT_MODE_CURSOR)
    if(wselect->cursor) x_pad_l=wselect->cursor->cursor->length;
  init_draw();
}
