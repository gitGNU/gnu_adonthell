...
{
#ifdef REVERSE_ALPHA
    if ((t == 255) && (alpha_ != 255) && data)
        SDL_SetAlpha (data, 0, 0);
    alpha_ = t;
#else
    if ((t == 0) && (alpha_ != 0) && data)
        SDL_SetAlpha (data, 0, 0);
    alpha_ = 255 - t;
#endif
}
...
void image::draw (s_int16 x, s_int16 y, drawing_area * da_opt = NULL)
{
    ...
    if (!dr.w || !dr.h)
        return;
    if (alpha () != 255)
        SDL_SetAlpha (data, SDL_SRCALPHA, alpha_);
...
void image::put_pix (u_int16 x, u_int16 y, u_int32 col)
{
    ...
    {
        case 1:
            *((u_int8 *) data->pixels + offset) = (u_int8) col;
            break;
        case 2:
            *((u_int16 *) data->pixels + offset) = (u_int16) col;
            break;
        case 4:
            *((u_int32 *) data->pixels + offset) = (u_int32) col;
            break;
        default:
            memcpy ((char *) data->pixels + (offset * bytes_per_pixel), &col,
                    bytes_per_pixel);
            break;
    }
}
