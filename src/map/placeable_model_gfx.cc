                                      ...
                                      gfx::surface * target) const
{
    ...
    gfx::screen::display.fillrect(x, y, st->area_length() * square_size, 1, 0xFFFFFF);
    gfx::screen::display.fillrect(x, y, 1, st->area_height() * square_size, 0xFFFFFF);
    gfx::screen::display.fillrect(x + st->area_length() * square_size - 1, y, 1, st->area_height() * square_size, 0xFFFFFF);
    gfx::screen::display.fillrect(x, y + st->area_height() * square_size - 1, st->area_length() * square_size, 1, 0xFFFFFF);
}

void placeable_model_gfx::put(ogzstream & file) const
{
    u_int32 s = Gfxs.size();
    s >> file;
...
void placeable_model_gfx::get(igzstream & file)
{
    u_int32 size;
