	...
	if((in_select_ && !selected_) || draw_brightness_)
	  {
	    ...
	    img.brightness(*theme_->scrollbar->back,level_brightness_);
            img.set_mask (true); 
	    img.draw(realx_ + length_ - theme_->scrollbar->back->length(),realy_,da_);
	    img.brightness(*theme_->scrollbar->bar,level_brightness_);
            img.set_mask (true); 
	    img.draw(1 + realx_ + length_ - theme_->scrollbar->back->length(),realy_+cursory_,da_);
	  }
	else
	  {
              theme_->scrollbar->back->set_mask (true); 
              theme_->scrollbar->bar->set_mask (true); 
              theme_->scrollbar->back->draw(realx_ + length_ - theme_->scrollbar->back->length(),realy_,da_);
              theme_->scrollbar->bar->draw(1 + realx_ + length_ - theme_->scrollbar->back->length(),realy_+cursory_,da_);
	  }
      return true;
    }
  return false;
