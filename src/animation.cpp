...
s_int8 animation::delete_image (u_int16 pos)
{
    ...
    for (j = frame.begin (); j != frame.end (); j++)
        if (j->image_nbr () >= pos)
            j->set_image_nbr (j->image_nbr () - 1);
#ifdef _EDIT_
    if (currentimage >= nbr_of_images ())
        currentimage = nbr_of_images () - 1;
    if (in_editor)
    {
        must_upt_label_frame_nbr = true;
        must_upt_label_frame_info = true;
...
s_int8 animation::delete_frame (u_int16 pos)
{
    ...
    frame.erase (i);
#ifdef _EDIT_
    if (currentframe_ >= nbr_of_frames ())
        currentframe_ = nbr_of_frames () - 1;
    if (in_editor)
    {
        must_upt_label_frame_nbr = true;
        must_upt_label_frame_info = true;
...
{
    screen::drawbox (x + xoffset (), y + yoffset (), length (), 1, 0xFFFFFF,
                     da_opt);
    screen::drawbox (x + xoffset (), y + yoffset () + height (), length (), 1,
                     0xFFFFFF, da_opt);
    screen::drawbox (x + xoffset () + length (), y + yoffset (), 1,
                     height () + 1, 0xFFFFFF, da_opt);
    screen::drawbox (x + xoffset (), y + yoffset (), 1, height () + 1,
                     0xFFFFFF, da_opt);
}
...
void animation::ed_add_frame ()
{
    ...
    {
        win_info *wi =
            new win_info (70, 40, th, font,

                          "You must have at least one image in your animation before thinking about inserting a frame!");
...
void animation::update_editor_keys ()
{
    ...
    if (input::has_been_pushed (SDLK_g))
        set_mode (IMAGE);
    // Frame mode
    if (input::has_been_pushed (SDLK_f))
        set_mode (FRAME);
    ...
    {
        if (mode == IMAGE)
            select_image (increase_image (currentimage));
        if ((mode == FRAME) && (!play_flag))
            if (!(SDL_GetModState () & KMOD_SHIFT))
            {
                ...
                set_frame_gapx (currentframe (),
                                frame[currentframe ()].gapx + 1);
    }
    ...
    {
        if (mode == IMAGE)
            select_image (decrease_image (currentimage));
        if ((mode == FRAME) && (!play_flag))
            if (!(SDL_GetModState () & KMOD_SHIFT))
            {
    ...
    if (testkey (SDLK_UP) && mode == FRAME && !play_flag)
        if (!(SDL_GetModState () & KMOD_SHIFT))
        {
            set_frame_delay (currentframe (),
                             frame[currentframe ()].delay () + 1);
        }
        else
            set_frame_gapy (currentframe (), frame[currentframe ()].gapy - 1);
    ...
    // Start playing
    if (input::has_been_pushed (SDLK_p))
        play ();
    // Stop playing
    if (input::has_been_pushed (SDLK_o))
        stop ();
    // Rewind
    ...
    {
        if (mode == IMAGE)
            delete_image (currentimage);
        if ((mode == FRAME) && (!play_flag))
            delete_frame (currentframe ());
    }
    ...
    if (testkey (SDLK_PAGEUP) && mode == FRAME && !play_flag)
        set_frame_alpha (currentframe (),
                         frame[currentframe ()].alpha () + 1);
    ...
    if (testkey (SDLK_INSERT) && mode == FRAME && !play_flag)
        set_frame_mask (currentframe (),
			frame[currentframe ()].is_masked () == true ? false : true);
}
...
void animation::update_editor ()
{
    ...
    // Update status
    if (must_upt_label_mode)
    {
        update_label_mode ();
        must_upt_label_mode = false;
    }
    if (must_upt_label_frame_nbr)
    {
        update_label_frame_nbr ();
        must_upt_label_frame_nbr = false;
    }
    if (must_upt_label_frame_info)
    {
        update_label_frame_info ();
        must_upt_label_frame_info = false;
    }
    if (must_upt_label_status)
    {
        update_label_status ();
