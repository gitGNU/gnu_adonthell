...
win_file_select::win_file_select (s_int16 tx, s_int16 ty, u_int16 tl,
                                  u_int16 th, win_theme * theme, win_font * wf, char *fext, char *bdir =
                                  NULL):win_container (tx, ty, tl, th, theme)
{
    font_ = wf;
    ...
    if (bdir)
    {
        if (cur_dir_[strlen (base_dir_) - 1] != '/')
            strcat (base_dir_, "/");
    ...
    if (chdir (cur_dir_) == 0)
    {
        if ((cwd = opendir (cur_dir_)))
        {
            while ((dirlist = readdir (cwd)))
            {
                ...
                tmplabel =
                    new win_label (0, 0,
                                   combo_file_->length () - space_between_border_ -
                                   theme_->scrollbar->back->length (), font_->height (),
                                   theme_, font_);
...
string win_file_select::wait_for_select (const Functor0 & updatefunc,
