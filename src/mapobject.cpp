...
s_int8 mapobject::insert_animation (animation * an, u_int16 pos)
{
    ...
    an->play ();
#ifdef _DEBUG_
    cout << "Added animation: " << nbr_of_animations () <<
        " total in mapobject.\n";
#endif
    return 0;
}
...
s_int8 mapobject::delete_animation (u_int16 pos)
{
    ...
    anim.erase (i);
#ifdef _DEBUG_
    cout << "Removed part: " << nbr_of_animations () <<
        " total in mapobject.\n";
#endif
#ifdef _EDIT_
    if (currentpart >= nbr_of_animations ())
        currentpart = nbr_of_animations () - 1;
    if (in_editor)
    {
...
#ifdef USE_PYTHON
void mapobject::set_schedule (string file)
{
    if (file == "")
    {
        schedule_file = "";
    ...
    string script = "scripts/schedules/" + file + ".py";
    ...
    if (f)
    {
        ...
        _node *n = PyParser_SimpleParseFile (f, (char *) script.c_str () ,
                                             Py_file_input);
...
#ifdef _EDIT_
s_int8 mapobject::put (ogzstream& file)
{
    u_int16 i;
...
s_int8 mapobject::save (string fname)
{
    u_int8 retvalue;
    ogzstream file (MAPOBJECTS_DIR + fname); 
...
void mapobject::zoom (u_int16 sx, u_int16 sy, mapobject * src)
{
    ...
    for (i = 0; i < src->nbr_of_animations (); i++)
    {
        ...
        an->set_offset ((src->anim[i]->xoffset () * sx) / src->length (),
			(src->anim[i]->yoffset () * sy) / src->height ());
        an->play ();
        anim.push_back (an);
    }
}
...
void mapobject::update_label_part ()
{
    ...
    {
        ostrstream temp;
        temp << "Current part: " << currentpart << "/" << nbr_of_animations ()
             << "\nOffset: " << anim[currentpart]->xoffset () <<  ", "
             << anim[currentpart]->yoffset () << "\nObj. size: "
             << length () << ", " << height ()
             << endl << (anim[currentpart]->is_empty ()? "Part is empty\n" : "")
             << ends; 
...
//         sprintf (label_txt, "No parts yet!");
        label_txt = "No parts yet!"; 
    label_part->set_text (label_txt.c_str ());
...
void mapobject::save ()
{
    ...
    if (save (s)) st = "Error saving " + s + "!"; 
    else st = s + " saved successfully!";
    file_name = s; 
    set_info_win (st);
    delete qw;
}
...
void mapobject::load ()
{
    ...
    win_file_select *wf =
        new win_file_select (60, 20, 200, 200, th, font, ".mobj",
                             MAPOBJECTS_DIR);
    string s =
        wf->wait_for_select (makeFunctor (*this, &mapobject::update_editor),
    ...
    if (t->load (s)) st = "Error loading " + s + "!"; 
    else
    {
...
void mapobject::new_animation ()
{
    ...
    {
        ostrstream temp;
        temp << "Insert at pos(0-" << nbr_of_animations ()
             << "): (Default " << nbr_of_animations () << ")" << ends; 
        string tmp = temp.str (); 
...
void mapobject::resize_grid ()
{
    ...
    for (i = 0; i < nbr_of_animations (); i++)
    {
        if (mx < anim[i]->length () + anim[i]->xoffset ())
            mx = anim[i]->length () + anim[i]->xoffset ();
        if (my < anim[i]->height () + anim[i]->yoffset ())
            my = anim[i]->height () + anim[i]->yoffset ();
    }
    rx = mx / 20 + (mx % 20 != 0);
    ry = my / 20 + (my % 20 != 0);
    if (rx == maptpl::length () && ry == maptpl::height ())
        return;
    resize (rx, ry);
    resize_view (9, 9);
}
...
inline bool testkey (SDLKey k)
{
    if (SDL_GetModState () & KMOD_CTRL)
        ...
        return ((input::has_been_pushed (k)));
}
...
void mapobject::update_editor_keys ()
{
    ...
    {
        show_grid = show_grid == true ? false : true;
        must_upt_label_part = true;
    }
    ...
    if (testkey (SDLK_KP_PLUS))
        if (SDL_GetModState () & KMOD_SHIFT)
        {
            ...
            set_currentpart (increase_part (currentpart));
    ...
    if (testkey (SDLK_KP_MINUS))
        if (SDL_GetModState () & KMOD_SHIFT)
        {
            ...
            set_currentpart (decrease_part (currentpart));
    ...
    if (testkey (SDLK_LEFT) && show_grid)
    {
        if (SDL_GetModState () & KMOD_SHIFT)
        {
            if (anim[currentpart]->xoffset ())
                set_part_xoffset (currentpart,
                                  anim[currentpart]->xoffset () - 1);
        }
    ...
    if (testkey (SDLK_UP) && show_grid)
    {
        if (SDL_GetModState () & KMOD_SHIFT)
        {
            if (anim[currentpart]->yoffset ())
                set_part_yoffset (currentpart,
                                  anim[currentpart]->yoffset () - 1);
        }
        else if (SDL_GetModState () & (KMOD_META | KMOD_ALT))
        {
...
void mapobject::draw_editor ()
{
    ...
    if (nbr_of_animations ())
    {
        draw_free (0 - (MAPSQUARE_SIZE * d_posx),
...
void mapobject::set_info_win (string text)
{
