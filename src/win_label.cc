...
win_label::win_label (s_int16 tx, s_int16 ty, u_int16 tl, u_int16 th,
                      win_theme * wthem, win_font * fo):win_base (tx, ty, tl, th, wthem)
{
...
s_int8 win_label::word_place (u_int16 cur_line_size, s_int16 w_size)
{
    ...
    if (w_size > length_)
        return 0;
    if (w_size > length_ - cur_line_size)
        return 2;
    return 1;
}
...
void win_label::init_draw_surface ()
{
    ...
    if (!auto_size_ && auto_height_)
    {
        ...
        while (i < texte_size_)
        {
            ...
            {
                if (curligne_ + font_->length () + 1 < length_)
                    curligne_ += font_->length () + 1;
                else
                {
                    curligne_ = 0;
                    tmpheight_ += font_->height ();
            ...
            switch (word_place (curligne_, (tmpwsize_ =
                                            word_size (i, wnbch_))))
            {
                case 0:
                    for (int j = i; j < wnbch_ + i; j++)
                    {
                        ...
                        {
                            if (curligne_ + (*font_)[texte_[j]].length () +
                                1 > length_)
                            {
                                curligne_ = 0;
                                tmpheight_ += font_->height ();
    ...
    {							//if autosize
        if (auto_size_)
        {
            ...
            {
                if (texte_[i] != '\n' && texte_[i] != ' '
                    && font_->in_table (texte_[i]))
                    curligne_ += (*font_)[texte_[i]].length () + 1;
                else if (texte_[i] == '\n')
                {
                    tmpheight_ += font_->height ();
                    if (curligne_ > tmplength_)
                        tmplength_ = curligne_;
                    curligne_ = 0;
                }
                else if (texte_[i] == ' ')
                    curligne_ += font_->length () + 1;
            ...
            if (blinkcursor_)
            {
                if (curligne_ + font_->cursor->length () > tmplength_)
                    tmplength_ = curligne_ + font_->cursor->length ();
            }
            else if (curligne_ > tmplength_)
                tmplength_ = curligne_;



            if (tmpheight_ != height_ + font_->height ()
                || tmplength_ != length_)
            {
...
void win_label::init_draw ()
{
    ...
    delete template_;
    template_ = new image (length_, height_); 
    ...
    while (i < texte_size_)
    {
        ...
        if (tmpheight_ >= template_->height ())
            break;
        switch (word_place (curligne_, (tmpwsize_ = word_size (i, wnbch_))))
        {
            case 0:
                for (j = i; j < wnbch_ + i; j++)
                {
                    if (font_->in_table (texte_[j]))
                    {
                        if (curligne_ + (*font_)[texte_[j]].length () + 1 >
                            length_)
                        {
                            curligne_ = 0;
                            tmpheight_ += font_->height ();
                        ...
                        if (tmpheight_ >= template_->height ())
                            break;
                        ((*font_)[texte_[j]]).draw (curligne_, tmpheight_, NULL, template_);
                        curligne_ += (*font_)[texte_[j]].length () + 1;
                    }
                }
                i += wnbch_;
    ...
    if (focus_ && blinkcursortimetodraw_ && blinkcursor_
        && template_->length () != 0)
    {
        //modify because error if not autosize
        if (auto_size_)
            font_->cursor->draw (curligne_, tmpheight_, NULL, template_);
        else
        {
            if (curligne_ + font_->cursor->length () > length_)
            {
...
bool win_label::draw ()
{
    if (win_base::draw ())
    {
        ...
        if (template_)
            ...
            {
                template_->set_mask (true); 
                template_->draw (realx_, realy_, da_);
            }
        draw_border ();
        detach_drawing_area ();
        return true;
...
bool win_label::update ()
{
    if (win_base::update ())
    {
        ...
        {
            if (blinkinc_++ > blinkingspeed_)
                blinkinc_ = 0;
            blinkcursortimetodraw_ = (blinkinc_ > (blinkingspeed_ >> 1));
            if (old != blinkcursortimetodraw_)
                init_draw ();
        }
        return true;
    }
