                         ...
                         0)
{
    ...
    if (px >= m_map->submap[currentsubmap]->length ||
        py >= m_map->submap[currentsubmap]->height)
        return -1;
                    ...
                    surface *target = NULL) const
{
    ...
    j0 = posy;
    ie =
        i0 + d_length + (offx != 0) < l->length ? i0 + d_length + (offx !=
                                                                   0) : l->length;
    je =
        j0 + d_height + (offy != 0) < l->height ? j0 + d_height + (offy !=
                                                                   0) : l->height;
    ...
    while (itc != characters_draw.end () || it != critical_draw.end ())
    {
        if (itc != characters_draw.end ())
        {
            if (it != critical_draw.end ())
            {
                ...
                {
                    it->draw (this, x, y, NULL, target);
                    it++;
                }
                else
                {
                    itc->base_tile->mchar->draw (this, x, y, target);
                    itc++;
                }
            }
            else
    ...
    {
        // Left overflow
        for (it = l->land[i0][j].tiles.begin ();
             it != l->land[i0][j].tiles.end () && *(it->base_tile) <= *it;
             it++)
            if (it->y == it->base_tile->y && it->x > it->base_tile->x)
                it->base_tile->draw (this, x, y, NULL, target);
        ...
        for (i = i0; i < ie; i++)
        {
            for (it = l->land[i][j].base_begin;
                 it != l->land[i][j].tiles.end () && *(it->base_tile) < *it;
                 it++);
            for (;
                 it != l->land[i][j].tiles.end () && *(it->base_tile) == *it;
                 it++)
        ...
        // Right overflow
        for (it = l->land[ie - 1][j].tiles.begin ();
             it != l->land[ie - 1][j].tiles.end (); it++)
            if (it->y == it->base_tile->y && it->x < it->base_tile->x)
                it->base_tile->draw (this, x, y, NULL, target);
        ...
        for (itc = characters_draw.begin (); itc != characters_draw.end ();
             itc++)
            itc->mchar->draw (this, x, y, target);
        characters_draw.clear ();
    }
    ...
    if (!l->land[i0][je - 1].tiles.empty ())
             ...
             it->y < it->base_tile->y; it--)
        {
            ...
            if (it == l->land[i0][je - 1].tiles.begin ())
                break;
        }
    ...
    for (i = i0; i < ie && i < l->length; i++)
    {
        if (!l->land[i][je - 1].tiles.empty ())
                 ...
                 it->y < it->base_tile->y; it--)
            {
                if (it->x == it->base_tile->x && it->y < it->base_tile->y)
                    critical_draw.push_front (*(it->base_tile));
                if (it == l->land[i][je - 1].tiles.begin ())
                    break;
            }
    ...
    if (!l->land[ie - 1][je - 1].tiles.empty ())
             ...
             it->y < it->base_tile->y; it--)
        {
            if (it->x < it->base_tile->x && it->y < it->base_tile->y)
                critical_draw.push_front (*(it->base_tile));
            if (it == l->land[ie - 1][je - 1].tiles.begin ())
                break;
        }
...
void mapview::update_label_square ()
{
        ...
        land[mapselect::posx][mapselect::posy].tiles.empty ())
    {
        list < mapsquare_tile >::iterator i =
            m_map->submap[currentsubmap]->
            land[mapselect::posx][mapselect::posy].tiles.begin ();
        u_int16 cpt = 0;
...
void mapview::add_submap ()
{
    ...
    {
        set_info_win
            ("You must have at least 1 object before adding a submap!");
        return;
    }
    m_map->add_submap ();
    m_map->nbr_of_submaps--;
...
void mapview::delete_submap ()
{
    if (m_map->nbr_of_submaps)
    {
        ...
        if (m_map->nbr_of_submaps)
        {
            ...
            current_tile =
                m_map->
                submap[currentsubmap]->land[mapselect::posx][mapselect::
                                                             posy].tiles.begin ();
            update_current_tile (*current_tile);
        }
    }
...
{
    if (currentobj < m_map->nbr_of_patterns - 1)
        currentobj++;
    else
        currentobj = 0;
    must_upt_label_object = true;
...
void mapview::draw_cursor ()
{
    ...
    if (mapselect::cursor_blink < CURSOR_BLINK_RATE)
        m_map->pattern[currentobj]->draw (s_posx +
                                          MAPSQUARE_SIZE * (mapselect::posx - mapselect::d_posx),
                                          s_posy + MAPSQUARE_SIZE * (mapselect::posy - mapselect::d_posy),
                                          da);
}

void mapview::draw_walkable (u_int16 x, u_int16 y, drawing_area * da_opt =
                             NULL)
{
    static u_int16 i, j;
    static u_int16 i0, j0, ie, je;
    ...
    for (j = j0; j < je; j++)
        for (i = i0; i < ie; i++)
        {
            ...
            if (!l->land[i][j].is_walkable_east ())
                screen::drawbox (rx * MAPSQUARE_SIZE - offx + MAPSQUARE_SIZE -
                                 1, ry * MAPSQUARE_SIZE - offy + 1, 1, MAPSQUARE_SIZE - 2,
                                 col);
...
void mapview::draw_editor ()
{
    ...
    container->draw ();
    if (m_map->nbr_of_patterns)
        m_map->mini_pattern[currentobj]->draw_free (245, 82);
    if (m_map->nbr_of_submaps)
    {
        if (!m_map->
            submap[currentsubmap]->land[mapselect::posx][mapselect::posy].
            tiles.empty ())
    ...
    if (must_upt_label_pos)
        update_label_pos ();
    if (must_upt_label_object)
        update_label_object ();
    if (must_upt_label_square)
        update_label_square ();
