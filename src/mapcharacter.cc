...
{
    dialog_engine *de = new dialog_engine (this, (char *) dialogue.c_str (), 0);
...
bool mapcharacter::can_go_north ()
{
    ...
    u_int16 ex =
        (posx - basex + mapselect::length () >=
         refmap->submap[submap]->length) ? refmap->submap[submap]->length -
        1 : posx - basex + mapselect::length ();
    u_int16 ey =
        (posy - basey + mapselect::height () >=
         refmap->submap[submap]->height) ? refmap->submap[submap]->height -
        1 : posy - basey + mapselect::height ();

    for (j = sy; j < ey; j++)
        for (i = sx; i < ex; i++)
        {
            if (placetpl[i - sx + ax][j - sy + ay].walkable)
                continue;
            if (!j)
                continue;
            if (!(refmap->submap[submap]->land[i][j].is_walkable_north () &&
                  refmap->submap[submap]->land[i][j -
                                                  1].is_walkable_south ()
...
{
    if (posy == refmap->submap[submap]->height - 1)
        return false;
    u_int16 i, j;
    u_int16 sx = (posx - basex < 0) ? 0 : posx - basex;
    u_int16 sy = (posy - basey < 0) ? 0 : posy - basey;
    ...
    for (j = sy; j < ey; j++)
        for (i = sx; i < ex; i++)
        {
            if (placetpl[i - sx + ax][j - sy + ay].walkable)
                continue;
            if (j == refmap->submap[submap]->height - 1)
                continue;
...
{
    if (posx == refmap->submap[submap]->length - 1)
        return false;
    u_int16 i, j;
    u_int16 sx = (posx - basex < 0) ? 0 : posx - basex;
    u_int16 sy = (posy - basey < 0) ? 0 : posy - basey;
    ...
    for (j = sy; j < ey; j++)
        for (i = sx; i < ex; i++)
        {
            if (placetpl[i - sx + ax][j - sy + ay].walkable)
                continue;
            if (i == refmap->submap[submap]->length - 1)
                continue;
...
void mapcharacter::draw (s_int16 x, s_int16 y, drawing_area * da_opt = NULL, surface * target = NULL)
{
...
void mapcharacter::draw (const mapview * mv, u_int16 x, u_int16 y, surface * target = NULL)
{
    u_int16 xdraw =
        ...
        mv->draw_offx;
    u_int16 ydraw =
        ((posy - mv->posy - basey) * MAPSQUARE_SIZE) + offy - mv->offy + y -

        mv->draw_offy;
    draw (xdraw, ydraw, mv->da, target);
}
    ...
    if (testkey (SDLK_LEFT) && show_grid)
    {
        if (SDL_GetModState () & KMOD_SHIFT)
        {
            if (anim[current_move]->xoffset ())
                set_anim_xoffset (current_move,
                                  anim[current_move]->xoffset () - 1);
        }
        else if (SDL_GetModState () & (KMOD_META | KMOD_ALT))
        {
            if ((maptpl::length () - 1) * MAPSQUARE_SIZE >=
                anim[current_move]->length () +
                anim[current_move]->xoffset ())
    ...
    if (testkey (SDLK_UP) && show_grid)
    {
        if (SDL_GetModState () & KMOD_SHIFT)
        {
            if (anim[current_move]->yoffset ())
                set_anim_yoffset (current_move,
                                  anim[current_move]->yoffset () - 1);
        }
        else if (SDL_GetModState () & (KMOD_META | KMOD_ALT))
        {
            if ((maptpl::height () - 1) * MAPSQUARE_SIZE >=
                anim[current_move]->height () +
                anim[current_move]->yoffset ())
