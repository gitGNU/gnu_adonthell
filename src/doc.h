...
/*! 
 ...
 readability is a correct indenting. Here are a few rules to follow:

 \li Block indenting is set to 4 characters. 
 ...
 code. Doxygen is a very powerfull documentation system that allows the programmer
 to quickly document his code with class references, diagrams (inheritance, class
 hierarchy, ...) and many other usefull things for a very limited effort, letting
 him concentrate on the code rather than the documentation. The programmer documentation
 ...
 EVERY file in adonthell MUST start with the following:
 \verbatim
 /*
   $Id:
 ...
 */
 ...
 that you are the copyright holder of the file. On the Copyright line must
 figure all the years this file has been worked on. Several persons may hold
 the copyright of a file. In this case, put one Copyright line per person.
 The $Id: line is for the CVS system. It will put usefull information on this
 ...
 Each class must have a little documentation block expliciting what it does.
 ...
 \verbatim
 /**
  * This very powerfull class blah blah....
 ...
 */
 ...
 the Class Hierarchy diagram, have it's own documentation page with automatically
 ...
 if it ever meet one. But the functions or operator you have overloaded must
 anyway be available from the Python interface. To make them available, you'll
 place a ifdef SWIG directive to declare another inlined function, with a different
 name (or an obvious name for an operator). An example is better than a long explanation:
 \verbatim
 class myclass
 {
 public:
    ...
    /**
     * Drawing method 2
     * @attention Not available from Python. Use draw_part () from Python instead.
     * @sa draw_part ()
     */ 
     void draw (int x, int y, int l, int h);
#endif
    /**
     * Synonym of draw (x, y, l, h) to guarantee it's access from Python.
     */ 
 ...
 Operators synonym functions should have explicit names. As an operator
