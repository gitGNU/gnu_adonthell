...
static void select_row_cb(GtkWidget * widget, gint row, gint column, GdkEventButton * bevent, gpointer data)
{
	...
	if (bevent)
	{
		if (bevent->type == GDK_2BUTTON_PRESS)
		{
    		...
    		wnd = (main_wnd *) gtk_object_get_user_data(GTK_OBJECT(widget));

	       	if (wnd) wnd->cur_dir = g_strdup (dirnode->path);

	        on_widget_destroy(gtk_widget_get_ancestor (GTK_WIDGET (widget), GTK_TYPE_WINDOW), NULL);
		}
	}
}
...
static void ok_clicked(GtkWidget * widget, GtkWidget * tree)
{
	...
	while (list_node)
	{
		...
		wnd = (main_wnd *) gtk_object_get_user_data(GTK_OBJECT(tree));
		if (wnd) wnd->cur_dir = g_strdup (dirnode->path);

		list_node = g_list_next(list_node);
	}
	
	on_widget_destroy(window, NULL);
}
...
GtkWidget *xmms_create_dir_browser(gchar * title, gchar * current_path, GtkSelectionMode mode, main_wnd *wnd)
{
	...
	gtk_object_set_user_data(GTK_OBJECT(tree), (gpointer) wnd);
	...
	gtk_signal_connect_object(GTK_OBJECT(cancel), "clicked", GTK_SIGNAL_FUNC(on_widget_destroy), GTK_OBJECT(window));
	...
	if (current_path && *current_path)
	{
		...
		node = gtk_ctree_find_by_row_data_custom(GTK_CTREE(tree), NULL, root_text, filetreeent_compare_func);
		do
		{
	...
	gtk_signal_connect(GTK_OBJECT(window), "destroy", GTK_SIGNAL_FUNC(on_widget_destroy), window);
